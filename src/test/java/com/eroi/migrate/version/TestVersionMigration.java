...
//		assertTrue(Execute.tableExists(versionTable));
		...
		// The fact that table from migration 1 does not exist while the tables from migration 5 do exist proofs that the migration continues the
		// existing migration
	...
	private boolean _columnExists(String columnName, String tableName) {
		...
		boolean result = Execute.columnExists(columnName, tableName) || Execute.columnExists(columnName.toLowerCase(), tableName)
	...
	private void _createOldStyleVersionTable(int version) throws SQLException {
		...
		if (!Execute.tableExists(tableName)) {
			query = "create table "+tableName+" (version int not null primary key)";
			Execute.executeStatement(getConnection(), query);
		}

		query = String.format("INSERT INTO %s (%s) VALUES (%d)", tableName, // INSERT INTO %s

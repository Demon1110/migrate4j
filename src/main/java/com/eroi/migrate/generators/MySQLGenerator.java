...
public class MySQLGenerator extends GenericGenerator {
    ...
    public String createTableStatement(Table table, String options) {
		...
		if (table.getTableComment()!=null) {
			 retVal.append(" COMMENT '").append(table.getTableComment()).append("'");
		}
	    retVal.append(";");
    ...
    protected String makeColumnString(Column column, boolean suppressPrimaryKey) {
    	...
    	if (column.getColumnComment()!=null&&!"".equals(column.getColumnComment().trim())) {
    		retVal.append(" COMMENT '").append(column.getColumnComment()).append("' ");
		}
